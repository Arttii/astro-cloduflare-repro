...
import { SITE, BLOG, APP } from '~/config.mjs';
...
const createFullPath = (...params) => params.filter((el) => !!el).join('/');
...
export const APP_BASE = cleanSlug(APP?.pathname);
...
export const getPermalink = (slug = '', type = 'page') => {
  ...
  switch (type) {
    ...
    case 'full':
      return createFullPath(`${APP_BASE}`, slug);

    case 'app':
      return createPath(basePathname, APP_BASE, _slug);
...
/** */
export const getAppPermaLink = () => getPermalink('app', '');
/** */
export const getUploadPermaLink = () => getPermalink('upload', 'app');

/** */
export const getSettingsPermaLink = () => getPermalink('settings', 'app');

/** */
export const getLoginPermaLink = () => getPermalink('login', '');
